...
func albumByArtist(name string) ([]Album, error) {
	...
	defer rows.Close()
	// Loop throughs rows, using Scan to assign column data to struct fields.
	for rows.Next() {
		var alb Album
...
func main() {
	...
	var err error
	db, err = sql.Open("mysql", config_db.FormatDSN())
	if err != nil {
		log.Fatal(err)
	}
	...
	// Hard-core ID 2 here to test the query.
	alb, err := albumByID(2)
	if err != nil {
		log.Fatal(err)
	}
